...
import com.brainSocket.khednima3ak3.ActivityEventPrompt;
		...
		try {
			...
			if(extras.containsKey("contentType")) {
				...
				if(contentType.equals("event")){
					...
					msg = context.getString(R.string.event_prompt_msg_part1) + " " + event.getPeer().getName()+ " "+ context.getString(R.string.event_prompt_msg_part2) ;
					...
					if(event.isHasAgree() == TASK_STATE.ACCEPTED && event.isComplete()){
						msg = context.getString(R.string.notification_event_completed_msg_part1) + " " + event.getPeer().getName()+ " "+ context.getString(R.string.notification_event_completed_msg_part2) ;
					}else if(event.isHasAgree() == TASK_STATE.ACCEPTED ){
						msg = context.getString(R.string.notification_event_accepted_msg_part1) + " " + event.getPeer().getName()+ " "+ context.getString(R.string.notification_event_msg_part2) ;
					}else if (event.isHasAgree() == TASK_STATE.REJECTED){
						msg = context.getString(R.string.notification_event_rejected_msg_part1) + " " + event.getPeer().getName()+ " "+ context.getString(R.string.notification_event_msg_part2) ;
					}
					
					displayPushNotification(context, msg, notificationIntent);
					
			}else if(contentType.equals("generalNotification")){
//				{contentType:"generalNotification",
//					content:{
//					title:"xx",
//					data:[
//					{dataType:"text",plainData:"ywebfyberfbrf",size:"2323"},
//					{dataType:"image",plainData:"http://xx.com/image.png",size:"121"},
//					]
//					date:"long",
//					seeMoreUrl:"http://xxxxxxx"
//					}
//					} 
...
/*			if(event != null){
				...
				// setup flag
				notificationIntent.putExtra("isPush", true);
				notificationIntent.setClass(context, ActivityEventPrompt.class);
				notificationIntent.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT | Intent.FLAG_ACTIVITY_SINGLE_TOP);
				notificationIntent.putExtra("sessionId", event.getSessionId());
				displayPushNotification(context, msg, notificationIntent);
