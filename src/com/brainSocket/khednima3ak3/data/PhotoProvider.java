...
public class PhotoProvider
{
	...
	{
		try {
			File cacheDir = KhedniApp.getAppContext().getExternalCacheDir();
			if (cacheDir == null){//cache dir not available
				cacheDir = KhedniApp.getAppContext().getCacheDir();
			}
			File thisCacheDir = new File(cacheDir.getPath(), "Khedni");
			...
			ImageLoaderConfiguration.Builder builder = new Builder(KhedniApp.getAppContext());
...
//			builder.memoryCache(new UsingFreqLimitedMemoryCache(2000000));
			...
			.showImageOnLoading(R.drawable.shape_transparent)
			.showImageForEmptyUri(R.drawable.default_male_user)
			.showImageOnFail(R.drawable.default_male_user)
			.displayer(new FadeInBitmapDisplayer(300))
			.considerExifParams(true)
			.cacheInMemory(true)
			.cacheOnDisc(true)
	...
	private void checkLastCacheClearTimestamp()
	{
		try {
			...
			long timenow = KhedniApp.getTimestampNow();
			// compare times
